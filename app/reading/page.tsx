...
import { READING_SPREADS } from '@/lib/spreads';
import type { Category, SpreadType, ReadingCard } from '@/types';
...
export default function ReadingPage() {
  ...
  const selectedSpread = READING_SPREADS.find((option) => option.value === spreadType);
  const cardCount = selectedSpread?.cardCount ?? 0;

  async function revealCardsSequentially(count: number, targetReadingId: string) {
    ...
    await delay(500);
    if (sequenceCancelledRef.current) return;
    router.push(`/reading/result?id=${targetReadingId}`);
  }
  ...
  async function handleDrawCards() {
    ...
    try {
      ...
      void revealCardsSequentially(data.cards.length, data.readingId);
    } catch (err) {
